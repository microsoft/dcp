...
func (k *Kubeconfig) GetData() (net.IP, int, string, error) {
	const InvalidPort = 0

	kubeContext, found := k.Config.Contexts[k.Config.CurrentContext]
	if !found {
		return nil, InvalidPort, "", fmt.Errorf("Kubeconfig file is invalid; the context named '%s' (current context) does not exist", k.Config.CurrentContext)
	}
...
func GetKubeconfigFromFlags(fs *pflag.FlagSet, port int32) (*Kubeconfig, error) {
	...
	return getKubeconfig(kubeconfigPath, port)
}
...
func EnsureKubeconfigFile(kubeconfigPath string) error {
	k, err := getKubeconfig(kubeconfigPath, 0)
	if err != nil {
		return err
	}
...
func getKubeconfig(kubeconfigPath string, port int32) (*Kubeconfig, error) {
	info, err := os.Stat(kubeconfigPath)
	...
	if err != nil {
		...
		// Create a new config
		config, err = createKubeconfig(port)
		if err != nil {
			return nil, err
		}
	} else if info.IsDir() {
...
func createKubeconfig(port int32) (*clientcmd_api.Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("kubeconfig file creation failed: could not obtain IP address(es) for localhost: %w", err)
	}
	if len(ips) == 0 {
		return nil, fmt.Errorf("kubeconfig file creation failed: could not obtain IP address(es) for localhost")
	}
