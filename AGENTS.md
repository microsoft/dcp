...
- Comments inside code blocks should be reserved for describing the INTENT for the code IF AND ONLY IF the intent is not  obvious from the code itself (for example: why certain API is called when it is seemingly not necessary, or why operations are done in certain order if it seems a different order might work even better). Do not add comments merely describing what the code does, or where the name of used members or methods indicates the intent (for example: do not attach "we are closing the socket" call to a `closeSocket()` method call).
