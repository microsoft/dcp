...
func (r *ContainerReconciler) buildImageWithOrchestrator(container *apiv1.Container, rcd *runningContainerData, log logr.Logger) func(context.Context) {
	return func(buildCtx context.Context) {
		...
		if err != nil {
			_ = r.releasePersistentContainerResourceLease(context.WithoutCancel(buildCtx), container, log, false)
			rcd.startupError = err
			rcd.startAttemptFinishedAt = metav1.NowMicro()
			rcd.containerState = apiv1.ContainerStateFailedToStart
		}
...
func (r *ContainerReconciler) startContainerWithOrchestrator(container *apiv1.Container, rcd *runningContainerData, containerName string, log logr.Logger, delay time.Duration) func(context.Context) {
	return func(startupCtx context.Context) {
		...
		err := func() error {
			...
			// Process any bind mounts to ensure there are valid sources on the host
			err = ensureContainerBindMountSources(container, log)
			if err != nil {
				return err
			}
			...
			// Runtimes that cannot copy files into a created-but-not-running container (e.g. the Apple
			// container runtime) bake the configured create-files and PEM certificates into a derived
			// image as additional layers, so the files are present in the container filesystem before
			// the entrypoint runs.
			...
			// Runtimes that can only write into a running container (e.g. the Apple container runtime)
			// cannot do this; they instead bake the files into the image as layers at creation time
			// (see bakeCreateFilesImageLayers above), so the copy here is skipped for them.
			if !r.orchestrator.CreateFilesRequiresRunningContainer() {
...
// computeContainerStartupInputs resolves environment variables and invocation args before container creation.
func (r *ContainerReconciler) computeContainerStartupInputs(ctx context.Context, container *apiv1.Container, rcd *runningContainerData, log logr.Logger) error {
	envErr := r.computeEffectiveEnvironment(ctx, container, rcd, log)
	if envErr != nil {
		if templating.IsTransientTemplateError(envErr) {
			log.Info("Could not compute effective environment for the Container, retrying startup...", "Cause", envErr.Error())
...
func ensureContainerBindMountSources(container *apiv1.Container, log logr.Logger) error {
	for _, volume := range container.Spec.VolumeMounts {
		...
		if errors.Is(volErr, os.ErrNotExist) {
			mkdirErr := os.MkdirAll(volume.Source, osutil.PermissionDirectoryOthersRead)
			if mkdirErr != nil {
				log.Error(mkdirErr, "Could not create bind mount source path", "Source", volume.Source, "Target", volume.Target)
				return mkdirErr
			}
		} else if volErr != nil {
...
// addContainerCreationLabels adds lifecycle and diagnostic labels used to manage or inspect the container later.
func (r *ContainerReconciler) addContainerCreationLabels(container *apiv1.Container, rcd *runningContainerData, log logr.Logger) (string, error) {
...
func (r *ContainerReconciler) applyContainerImageLayers(ctx context.Context, rcd *runningContainerData, extraLayers []apiv1.ImageLayer, log logr.Logger) (string, error) {
	...
	// Combine the layers declared in the run spec with any layers baked at creation time (e.g. Apple
	// container create-files). Do not mutate rcd.runSpec.ImageLayers: the container creation block may
	// retry, and mutating the spec would append the extra layers more than once.
	...
	// the derived image is expensive, so skipping it when the inputs are unchanged avoids redundant
	// work across runs. A miss here is expected (first run, or inputs changed) and falls through to
	// building.
...
func (r *ContainerReconciler) releasePersistentContainerResourceLease(
	ctx context.Context,
	container *apiv1.Container,
	log logr.Logger,
	...
	if releaseErr != nil {
		if errors.Is(releaseErr, statestore.ErrResourceLeaseNotHeld) {
			logResourceLeaseNotHeld(log, suppressNotHeldLog, container.GetLeaseKey(), "Persistent container resource lease was not held")
			return releaseErr
		}
...
// (e.g. the Apple container runtime) bake these layers into a derived image at creation time, so
// injected files — including TLS certificates an entrypoint reads at startup — are present before the
// container starts.
...
func (r *ContainerReconciler) handleContainerNetworkConnections(
	...
	if len(notConnected) > 0 {
		log.V(1).Info("Container became disconnected from some networks, updating status...", "DisconnectedNetworks", notConnected)
	}
	if len(newlyConnected) > 0 {
		log.V(1).Info("Container became connected to new networks, updating status...", "ConnectedNetworks", newlyConnected)
	}
...
func (r *ContainerReconciler) ensureContainerNetworkConnections(
	...
	if container.Spec.Networks == nil || containers.UsesSingleNetwork(r.orchestrator) || container.Spec.Stop || !container.DeletionTimestamp.IsZero() || rcd.containerState == apiv1.ContainerStateFailedToStart {
		// Delete all connections if no networks are defined, stop has been requested, or the container is being deleted.
		// Single-network runtimes (e.g. Apple container) have no custom networks to connect to: every
		// container is already on the default network, so no ContainerNetworkConnection objects are created.
		//
