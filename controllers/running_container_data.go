...
type runningContainerData struct {
	...
	// Filesystem path of the terminal HMP v1 Unix domain socket (the resolved path, which may have
	// been generated by DCP when the spec left UDSPath empty). Only set for containers with a terminal.
...
func (rcd *runningContainerData) Clone() *runningContainerData {
	clone := runningContainerData{
		...
		ptp:                    rcd.ptp,
		connMgr:                rcd.connMgr,
		terminalSocketPath:     rcd.terminalSocketPath,
	}
...
func (rcd *runningContainerData) UpdateFrom(other *runningContainerData) bool {
	...
	if other.terminalSocketPath != "" && rcd.terminalSocketPath != other.terminalSocketPath {
		rcd.terminalSocketPath = other.terminalSocketPath
		updated = true
	}
...
func (rcd *runningContainerData) applyTo(ctr *apiv1.Container, log logr.Logger) objectChange {
	...
	if rcd.terminalSocketPath != "" && ctr.Status.TerminalSocketPath != rcd.terminalSocketPath {
		ctr.Status.TerminalSocketPath = rcd.terminalSocketPath
		change |= statusChanged
	}
