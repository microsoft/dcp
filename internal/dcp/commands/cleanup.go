...
func cleanupPersistentVolumeRecord(
	...
	resourceID, _, cleanupErr := withCurrentPersistentVolumeRecord(ctx, workloadID, stateStore, leaseOwner, record, func(ctx context.Context, currentRecord *statestore.PersistentVolumeRecord) error {
		...
		removeErr := removePersistentVolume(ctx, orchestrator, currentRecord.VolumeName)
		if removeErr != nil {
			return removeErr
		}
...
func removePersistentVolume(ctx context.Context, orchestrator containers.ContainerOrchestrator, volumeName string) error {
