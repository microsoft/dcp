...
func addMonitorFlags(cmd *cobra.Command) error {
	...
	cmd.PersistentFlags().Var(flags.NewTimeFlag(&monitorProcessStartTime, osutil.RFC3339MiliTimestampFormat), "monitor-identity-time", "Specifies the identity time of the process to monitor. If omitted, identity is resolved once at command startup. The time format is RFC3339 with millisecond precision, for example "+osutil.RFC3339MiliTimestampFormat)
