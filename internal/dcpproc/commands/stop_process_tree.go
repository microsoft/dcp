...
func stopProcessTree(log logr.Logger) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		_, procErr := process.FindWaitableProcess(process.NewHandle(stopPid, stopProcessStartTime))
		if procErr != nil {
		...
		stopErr := process.StopViaConsole(log, pe, process.NewHandle(stopPid, stopProcessStartTime), stopOptions...)
		if stopErr != nil {
