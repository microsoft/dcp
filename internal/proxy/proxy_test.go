...
func TestTCPClientCanSendDataBeforeEndpointsExist(t *testing.T) {
	...
	port, err := networking.GetFreePort(commonapi.PortProtocolTCP, networking.IPv4LocalhostDefaultAddress, log)
	require.NoError(t, err)
	config := ProxyConfig{
		Endpoints: []Endpoint{
			{Address: networking.IPv4LocalhostDefaultAddress, Port: int32(port)},
...
func TestTCPParkedConnectionMaxCountLimit(t *testing.T) {
	...
	proxy := newNetProxy(commonapi.PortProtocolTCP, networking.IPv4LocalhostDefaultAddress, 0, ctx, logr.Discard())
...
func TestTCPProxyContinuousStream(t *testing.T) {
	...
	serverPort, portErr := networking.GetFreePort(commonapi.PortProtocolTCP, networking.IPv4LocalhostDefaultAddress, log)
...
func setupTcpProxy(t *testing.T, ctx context.Context) (*netProxy, *testutil.MockLoggerSink) {
	...
	proxy := newNetProxy(commonapi.PortProtocolTCP, networking.IPv4LocalhostDefaultAddress, 0, ctx, logr.New(logSink))
	require.NotNil(t, proxy)
...
func setupUdpProxy(t *testing.T, ctx context.Context) (*netProxy, *testutil.MockLoggerSink) {
	logSink := testutil.NewMockLoggerSink()
	proxy := newNetProxy(commonapi.PortProtocolUDP, networking.IPv4LocalhostDefaultAddress, 0, ctx, logr.New(logSink))
	require.NotNil(t, proxy)
