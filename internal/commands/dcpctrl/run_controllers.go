...
func NewRunControllersCommand() *cobra.Command {
...
func runControllers(cmd *cobra.Command, _ []string) error {
	// ctlrruntime.SetLogger() was already called by main()
	log := runtimelog.Log.WithName("dcpctrl")
	...
	mgr, err := getManager(cmd.Context(), log)
	if err != nil {
